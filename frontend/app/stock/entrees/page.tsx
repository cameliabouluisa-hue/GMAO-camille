...
function getSearchText(entree: StockEntree): string {
  const articles =
    entree.lignes
      ?.map((ligne) => {
        return [
...
export default function StockEntreesPage() {
  ...
  const filteredEntrees = useMemo(() => {
    ...
    return entrees.filter((entree) => {
      const matchesSearch = !q || getSearchText(entree).includes(q);

      const matchesStatut =
        statut === 'all' || String(entree.statut) === statut;

      return matchesSearch && matchesStatut;
    });
  }, [entrees, search, statut]);

  const stats = useMemo(() => {
    return {
      total: entrees.length,
      validees: entrees.filter((entree) => entree.statut === 'VALIDEE').length,
      lignes: entrees.reduce(
        (total, entree) => total + (entree.lignes?.length ?? 0),
        0,
      ),
      quantite: entrees.reduce(
        (total, entree) => total + getTotalQuantite(entree),
        0,
      ),
    };
  }, [entrees]);
    ...
    <main className="min-h-[calc(100vh-96px)] bg-[#f5f7fb] px-5 py-6">
      <section className="mx-auto max-w-[1450px] space-y-5">
        ...
        <StockEntreeListOptionB
          ...
          total={entrees.length}
          loading={loading}
          onView={(id) => router.push(`/stock/entrees/${id}`)}
        />
      </section>
    </main>
  );
